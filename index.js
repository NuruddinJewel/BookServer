...
async function run() {
    try {
        ...
        // Transaction History (GET) - purchasesCollection theke real data
        ...
        app.get('/admin/analytics', async (req, res) => {
            try {
                ...
                allPurchases.forEach((p) => {
                    ...
                    monthlySales[key] = (monthlySales[key] || 0) + (p.amount || 0);
                });

                const sales = monthNames
                    .filter((m) => monthlySales[m] !== undefined)
                    .map((m) => ({ month: m, sales: monthlySales[m] }));

                // Genre breakdown (ebooksCollection)
                const genreAgg = await ebooksCollection.aggregate([
                    { $group: { _id: "$category", value: { $sum: 1 } } },
                    { $project: { name: "$_id", value: 1, _id: 0 } }
                ]).toArray();

                res.status(200).json({
                    totals: { totalUsers, totalWriters, totalEbooks, totalSold, totalRevenue },
                    sales,
                    genres: genreAgg,
                });
            } catch (error) {
